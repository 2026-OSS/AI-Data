# YOLOv5 and YOLOv11 Training Environment
...
YOLOv5와 YOLOv11 모두 Roboflow에서 YOLO 형식으로 Export한 데이터셋을 사용한다.
...
Version 번호는 실제 학습에 사용할 Roboflow Version에 맞춰 변경한다.
...
YOLOv5를 `/content/yolov5`에서 실행하는 경우, Roboflow 다운로드 위치에 따라 `data.yaml` 경로를 보정한다.

```python
from pathlib import Path

dataset_dir = Path("./AI-Picture-Book-Object-Detection-6")
yaml_path = dataset_dir / "data.yaml"
...
!python train.py \
  ...
  --batch 16 \
  --epochs 3 \
  --data ./AI-Picture-Book-Object-Detection-6/data.yaml \
  --weights yolov5s.pt \
  --device 0 \
...
!python detect.py \
  ...
  --source AI-Picture-Book-Object-Detection-6/test/images \
  --img 640 \
  --conf 0.25 \
...
```bash
yolo detect train model=yolo11n.pt data=/path/to/data.yaml epochs=3 imgsz=640 batch=16 device=0
```
